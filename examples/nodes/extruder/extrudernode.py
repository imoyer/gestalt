...
#----- SERVICE ROUTINES -----------------
	...
	class svcTemperature(functions.serviceRoutine):
		class actionObject(core.actionObject):
			def init(self):
				self.setPacket({})
				self.commitAndRelease()
	...
	class svcSetTemp(functions.serviceRoutine):
		class actionObject(core.actionObject):
			def init(self, temp):
	...
	class svcExtrude(functions.serviceRoutine):
		class actionObject(core.actionObject):
			def init(self, temp):
				self.setPacket({})
				self.commitAndRelease()
				self.waitForChannelAccess()
				if self.transmitPersistent():
					notice(self.virtualNode, "extruding at bla per second")
					return True
				else:
					notice(self.virtualNode, "can't extrude")
					return False
